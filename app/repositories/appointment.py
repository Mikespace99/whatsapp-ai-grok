...
def list_appointments(
    ...
    """
    Elenco appuntamenti del tenant in un intervallo di date (inclusive).
    date_from / date_to: 'YYYY-MM-DD'
    Include nome cliente se presente (join customers).
    """
    sb = get_supabase()
    try:
        res = (
            sb.table("appointments")
            .select("id, customer_id, phone_number, service, appointment_date, appointment_time, status, notes, google_event_id, customers(full_name, phone_number)")
        ...
        for r in rows:
            ...
            client_name = (cust.get("full_name") if isinstance(cust, dict) else None) or r.get("phone_number") or "Cliente"
            time_str = (r.get("appointment_time") or "")[:5]
            out.append({
                "id": r["id"],
                "date": r.get("appointment_date"),
                "start": time_str,
                "client": client_name,
                "phone": r.get("phone_number") or (cust.get("phone_number") if isinstance(cust, dict) else None),
                "service": r.get("service") or "Appuntamento",
        ...
        print(f"[appointment_repo] Errore list_appointments: {e}")
        return []
...
def get_appointment(tenant_id: str, appointment_id: str) -> dict | None:
    ...
    try:
        ...
        print(f"[appointment_repo] Errore get_appointment: {e}")
        return None
